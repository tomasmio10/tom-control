...
export function SellerCommissionDetailPage() {
  ...
  return <>
    ...
    {loading ? <div className="catalog-loading" role="status"><span /><strong>Cargando detalle desde Supabase…</strong></div> : detail && <>
      ...
      <section className="panel commission-detail-panel"><div className="panel-heading"><div><h2>Ventas que generaron comisión</h2><p>La comisión se genera proporcionalmente al recaudo; los pedidos cancelados no generan saldo pagable.</p></div></div><div className="table-wrap"><table><thead><tr><th>Pedido</th><th>Fecha</th><th>Estado</th><th>Venta</th><th>Cobrado</th><th>Comisión total</th><th>Comisión generada</th><th>Estado comisión</th></tr></thead><tbody>{detail.sales.map((sale) => { const commissionState = getSaleCommissionState(sale.isPayable, detail); return <tr key={sale.orderId}><td><Link to={`/pedidos/${sale.orderId}`}><strong>#{sale.orderNumber}</strong></Link></td><td>{formatDate(sale.createdAt)}</td><td><span className={`status ${sale.status}`}>{statusLabels[sale.status]}</span></td><td className="money">{formatCurrency(sale.saleTotal)}</td><td className="money collected-money">{formatCurrency(sale.amountPaid)}</td><td className="money">{formatCurrency(sale.sellerCommissionAmount)}</td><td className="money due-money">{formatCurrency(sale.generatedCommissionAmount)}</td><td><span className={`commission-treatment ${commissionState.tone}`}>{commissionState.label}</span></td></tr> })}</tbody></table>{!detail.sales.length && <div className="empty-state">No hay ventas con información financiera para este vendedor.</div>}</div></section>
